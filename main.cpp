...
int main(int argc, char *argv[]) {
  ...
  if (me == THEBOSS) // this is the big-data-owner
  {
    BigSignal = new float[NUMELEMENTS +
                          MAXSHIFTS]; // so we can duplicate part of the array
  ...
  if (me == THEBOSS) {
    for (int dst = 0; dst < NumCpus; dst++) {
      if (dst != THEBOSS) {
        MPI_Send(&BigSignal[dst * PPSize], PPSize + MAXSHIFTS, MPI_FLOAT, dst,
                 TAG_SCATTER, MPI_COMM_WORLD);
      }
    }
  } else {
    MPI_Recv(PPSignal, PPSize + MAXSHIFTS, MPI_FLOAT, THEBOSS, TAG_SCATTER,
  ...
  if (me == THEBOSS) {
    ...
    for (int src = 0; src < NumCpus; src++) {
      if (src != THEBOSS) {
        MPI_Recv(tmpSums, MAXSHIFTS, MPI_FLOAT, src, TAG_GATHER, MPI_COMM_WORLD,
                 &status);
        for (int s = 0; s < MAXSHIFTS; s++)
          BigSums[s] += tmpSums[s];
      }
